...
@Component({
  selector: 'app-add-new-identification',
  templateUrl: './add-new-identification.component.html',
})
export class AddNewIdentificationComponent implements OnInit{
  ...
  numberTypeCodeList=[];
  constructor(private modalService: BsModalService, private comapanyAccService:CompanyServiceService,private fb:FormBuilder) { }
  ngOnInit() {
    this.createForm();
  }
  ...
  onSubmit(){
    if(this.form.valid)
    {
      ...
      this.comapanyAccService.addTaxNumber(this.newTaxNumber).subscribe((i:any)=>{
